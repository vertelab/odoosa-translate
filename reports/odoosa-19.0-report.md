# odoosa-report — 19.0 — 2026-08-31 05:33

**Totalt:** {'override': 917, 'converged': 120, 'conflict': 0, 'new-correct': 0, 'new-corrected': 0, 'new-ours': 0, 'new-manual': 0, 'flagged': 0, 'removed': 0, 'noop': 64331}

## account
- override: 113
- converged 🎉: 32
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3293
  · [override] . The journal entries need to be computed by Odoo before bei → '. Beloppen i verifikationerna måste berä'
  · [override] A Ledger group name must be unique per company. → 'Namnet på en huvudboksgrupp måste vara u'
  · [override] A fiscal position with a foreign VAT already exists in this  → 'En skatteområde med momsregistrering i u'
  · [override] Account %(account)s will be split in %(num_accounts)s, one f → 'Kontot %(account)s kommer att delas upp '
  · [override] Account %s cannot be unmerged as it already belongs to a sin → 'Kontot %s kan inte delas upp eftersom de'
  · [override] All selected moves for reversal must belong to the same comp → 'Alla markerade rättningsrörelser måste v'
  · [override] Always positive amount concerned by this matching expressed  → 'Enbart positivt belopp kopplat till denn'
  · [override] Amount Company Currency Signed → 'Undertecknat belopp i bolagets valuta'
  · [override] Amount to Pay (company currency) → 'Belopp att betala (bolagets valuta)'
  · [override] An account is part of a ledger allowing your company
        → 'Ett konto är en del av ett huvudbok som '
  · [override] Analytic Account → 'Objektkonto'
  · [override] Analytic Accounting → 'Objektredovisning'
  · [override] Analytic Accounts → 'Objektkonton'
  · [override] Analytic Distribution → 'Objektfördelning'
  · [override] Analytic Distribution Model → 'Objektfördelning'
  · [override] Analytic Distribution Models → 'Objektfördelning'
  · [override] Analytic Filter → 'Objektfilter'
  · [override] Analytic Lines → 'Objektrader'
  · [override] Analytic Plan's Applicabilities → 'Objektreglers användningsgrad'
  · [override] Analytic Plans → 'Objektregler'
  · [override] Analytic Precision → 'Objektprecision'
  · [override] Analytic Reporting → 'Objektrapporter'
  · [override] Analytic lines → 'Objektrader'
  · [override] Bank & Cash accounts cannot be shared between companies. → 'Bank- och kassakonton kan inte delas mel'
  · [override] Bank Account Number to which the invoice will be paid. A Com → 'Kontonummer för det bankkonto som faktur'
  · [override] Belongs to the same company as %s. → 'Tillhör samma bolag som %s.'
  · [override] Business licenses and permits that your company must pay to  → 'Bolagslicenser och tillstånd som ditt bo'
  · [override] Cannot change Price Tax computation method on a company that → 'Det är inte möjligt att ändra metoden so'
  · [override] Cannot find a chart of accounts for this company, You should → 'Ingen kontoplan hittades för det här bol'
  · [override] Companies → 'Bolag'
  · [override] Companies that refers to partner → 'Bolag kopplade till partner'
  · [override] Company → 'Bolag'
  · [override] Company  → 'Bolag'
  · [override] Company Bank Account → 'Bolagets bankkonto'
  · [override] Company Country → 'Bolagets land'
  · [override] Company Currency → 'Bolagets valuta'
  · [override] Company Document Layout → 'Bolagets dokumentlayout'
  · [override] Company Fiscal Country Code → 'Landskod för bolagets skatterättsliga he'
  · [override] Company Registry Placeholder → 'Platshållare för bolagsregister'
  · [override] Company Storno Accounting → 'Bolag med Storno-bokföring'
  · [override] Company Vat Placeholder → 'Platshållare för bolagets momsregistreri'
  · [override] Company has a chart of accounts → 'Bolaget har en kontoplan'
  · [override] Company related to this journal → 'Bolag relaterade till denna journal'
  · [override] Company's personnel costs excluding compensations, contribut → 'Bolagets personalkostnader exklusive tjä'
  · [override] Compensation paid to employees and workers employed by the c → 'Tjänsteersättning till anställda och arb'
  · [override] Countries for which the company has a VAT number → 'Länder för vilka bolaget har ett momsreg'
  · [override] Currency rate from company currency to document currency. → 'Växelkurs vid omvandling från bolagets v'
  · [override] Default Incoterm of your company → 'Standard incotermer för bolaget'
  · [override] Default on whether the sales price used on the product and i → 'Standard om huruvida det försäljningspri'
  · [override] Define which company can select the multi-ledger in report f → 'Ange vilket bolag som kan välja flerbola'
  · [override] Entries can only be created for a single company at a time. → 'Verifikat kan endast skapas för ett bola'
  · [override] Entries don't belong to the same company: %s → 'Följande verifikat tillhör inte samma bo'
  · [override] Expenses made for the repair and maintenance of company cars → 'Kostnader för reparationer och underhåll'
  · [override] Items With Missing Analytic Distribution → 'Objekt som saknar objektfördelning'
  · [override] Journal codes must be unique per company. → 'Journalkoder måste vara unika för varje '
  · [override] Journal where the opening entry of this company's accounting → 'Journal där bolagets ingående balans har'
  · [override] Main currency of the company. → 'Bolagets huvudsakliga valuta.'
  · [override] Main currency of your company → 'Bolagets huvudsakliga valuta'
  · [override] Mapping of account codes per company → 'Kartläggning av kontokoder per bolag'
  · [override] Multi-Company → 'Flera bolag'
  · [override] No journal could be found in company %(company_name)s for an → 'Ingen journal hittades i bolaget %(compa'
  · [override] Onboarding Company Data → 'Introduktion till bolagsdata'
  · [override] One or more lines require a 100% analytic distribution. → 'En eller flera rader kräver en objektför'
  · [override] Overrides the Company's default on whether the price you use → 'Gör det möjligt att bortse från bolagets'
  · [override] Partner Company Registry Placeholder → 'Platshållare för partner i bolagsregiste'
  · [override] Premiums paid for insurance coverage on company-owned or lea → 'Premier för fordonsförsäkringar för ford'
  · [override] Select a product category which will use analytic account sp → 'Välj ett specifikt objektkonto för en vi'
  · [override] Select a product for which the analytic distribution will be → 'Välj en specifik produkt för vilken obje'
  · [override] Set Company Data → 'Ange bolagsdata'
  · [override] Set your company data → 'Ange bolagsuppgifter'
  · [override] Set your company's data for documents header/footer. → 'Ställ in information om ditt bolag som s'
  · [override] Tax current account used as a counterpart to the Tax Closing → 'Skattekontot används för att beräkna utg'
  · [override] Taxes in company currency → 'Skatter i bolagets valuta'
  · [override] Technical field containing the countries for which this comp → 'Tekniskt fält som innehåller de länder f'
  · [override] Technical field giving the date the company lock date at the → 'Tekniskt fält som anger bolagets låsning'
  · [override] The amount expressed in the secondary currency must be posit → 'Beloppet som uttrycks i den sekundära va'
  · [override] The bank account of a bank journal must belong to the same c → 'Bankkontot kopplat till en bankjournal m'
  · [override] The bank account of your company is not trusted. Please ask  → 'Bolagets angivna bankkonto är ej betrott'
  · [override] The code must be set for every company to which this account → 'Koden måste anges för varje bolag som de'
  · [override] The company this distribution line belongs to. → 'Det bolag som denna fördelningsrad är ko'
  · [override] The country to use the tax reports from for this company → 'Landet från vilket skatterapporter ska a'
  · [override] The entry is using accounts (%(accounts_codes_names)s) from  → 'Verifikatet använder sig av konton (%(ac'
  · [override] The following accounts must be assigned to at least one comp → 'Följande konton måste vara kopplade till'
  · [override] The holder of a journal's bank account must be the company ( → 'Den angivna kontoinnehavaren för en jour'
  · [override] The journal entry containing the initial balance of all this → 'Verifikatet som innehåller ingående sald'
  · [override] The partners of the journal's company and the related bank a → 'Uppgifterna som angivits om de partners '
  · [override] The residual amount on a journal item expressed in its curre → 'Det återstående beloppet för en verifika'
  · [override] The residual amount on a journal item expressed in the compa → 'Det återstående beloppet för en verifika'
  · [override] The tax ID of your company in the region mapped by this fisc → 'Skatteregistreringsnumret för ditt bolag'
  · [override] There is no tax cash basis journal defined for the '%s' comp → 'Det finns ingen skattejournal enligt kon'
  · [override] This allows accountants to manage analytic and crossovered b → 'Detta gör det möjligt för revisorer att '
  · [override] This analytic distribution will apply to all financial accou → 'Objektfördelningen gäller för alla finan'
  · [override] This entry contains one or more taxes that are incompatible  → 'Det här verifikatet innehåller en eller '
  · [override] Use Company Selector → 'Använd bolagsväljare'
  · [override] We can't leave this document without any company. Please sel → 'Vi kan inte låta det här dokument vara u'
  · [override] We cannot find a chart of accounts for this company, you sho → 'Vi kan inte hitta bolagets kontoplan, du'
  · [override] You can't change the company of your journal since there are → 'Det är inte möjligt att byta journalens '
  · [override] You can't change the company of your tax since there are som → 'Det är inte möjligt att byta skattens an'
  · [override] You can't create a new payment without an outstanding paymen → 'Det är inte möjligt att skapa en ny beta'
  · [override] You can't create payments for entries belonging to different → 'Det är inte möjligt att skapa betalninga'
  · [override] You can't unlink this company from this account since there  → 'Det är inte möjligt att koppla bort det '
  · [override] You cannot change the currency of the company since some jou → 'Det är inte möjligt att ändra bolagets v'
  · [override] You cannot post an entry with an archived analytic account:  → 'Det är inte möjligt att bokföra ett veri'
  · [override] You cannot use this wizard on journal entries belonging to d → 'Det är inte möjligt att använda den här '
  · [override] You do not have the right to perform this operation as you d → 'Du har inte behörighet att genomföra den'
  · [override] You have to configure the 'Exchange Gain or Loss Journal' in → "Du bör ställa in en journal för 'vinst e"
  · [override] You must specify the Profit Account (company dependent) → 'Du måste ange ett vinstkonto (beroende p'
  · [override] You should configure the 'Gain Exchange Rate Account' in you → "Du bör ställa in ett 'konto för vinst vi"
  · [override] You should configure the 'Loss Exchange Rate Account' in you → "Du bör ställa in ett 'konto för förluste"
  · [override] e.g. My Company → 't.ex. Mitt bolag'
  · [override] This will update all taxes and accounts based on the current → 'Detta kommer att uppdatera alla skatter '
  · [override] <strong>This currency has already been used to generate acco → '<strong>Denna valuta har redan använts f'
  · [override] Write a company name to <b>create one</b> or <b>see suggesti → 'Skriv ett bolagsnamn för att <b>skapa et'
## account_add_gln
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 6
## account_check_printing
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 73
  · [override] Companies → 'Bolag'
## account_debit_note
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 48
## account_edi
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 73
## account_edi_proxy_client
- override: 3
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 33
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] This company has an active user already created for this EDI → 'Bolaget har redan en aktiv användare för'
## account_edi_ubl_cii
- override: 6
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 326
  · [override] %s should have a valid KBO/BCE number in the Company ID fiel → '%s bör ha ett giltigt KBO/BCE-nummer ang'
  · [override] Belgian Company Registry → 'Bolagsregister i Belgien'
  · [override] Estonia Company code → 'Bolagsnummer i Estland'
  · [override] Please fill in your company's VAT or Peppol Address to gener → 'Vänligen fyll i bolagets momsregistrerin'
  · [override] The Peppol endpoint is not valid. It should contain exactly  → 'Peppol-destinationen är inte giltig. Den'
  · [override] %s should have a KVK or OIN number set in Company ID field o → '%s bör ha ett KVK- eller OIN-nummer angi'
## account_fleet
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 22
## account_payment
- override: 1
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 125
  · [override] Overdue invoices should share the same company. → 'Förfallna fakturor bör ha samma bolag an'
## account_peppol
- override: 22
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 373
  · [override] <table border="0" cellpadding="0" cellspacing="0" style="pad → '<table border="0" cellpadding="0" cellsp'
  · [override] Active Parent Company → 'Aktivt överordnat bolag'
  · [override] Active Parent Company Name → 'Namn på aktivt överordnat bolag'
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Name → 'Bolagets namn'
  · [override] Company information → 'Bolagsinformation'
  · [override] I reckon I'm allowed to register this company. → 'Jag försäkrar om att jag har tillåtelse '
  · [override] Is Branch Company → 'Är en filial av ett bolag'
  · [override] Parent Company → 'Överordnat bolag'
  · [override] Peppol ID should be different from main company. → 'Peppol-ID bör skilja sig från huvudbolag'
  · [override] Peppol Parent Company → 'Peppol överordnat bolag'
  · [override] Peppol Parent Company Name → 'Peppol namn på överordnat bolag'
  · [override] Peppol Use Parent Company → 'Peppol använd överordnat bolag'
  · [override] Please select a country for your company. → 'Vänligen välj ett land för ditt bolag.'
  · [override] Register this company on peppol → 'Registrera bolaget hos peppol'
  · [override] Selected Company → 'Utvalda bolag'
  · [override] Send from parent company → 'Skicka från överordnat bolag'
  · [override] You are about to register with your VAT number. Make sure yo → 'Du är på väg att använda ett momsregistr'
  · [override] Your company is already registered on an Access Point (%s) f → 'Ditt bolag är redan registrerat på en Åt'
  · [override] [PEPPOL-EN16931-R020] An electronic address (EAS) must be pr → '[PEPPOL-EN16931-R020] En digital adress '
  · [override] The recommended identification method for Belgium is your Co → 'Den rekommenderade identifieringsmetoden'
## account_peppol_advanced_fields
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 27
## account_peppol_response
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 70
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
## account_qr_code_emv
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 18
## account_qr_code_sepa
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 8
## account_tax_python
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 24
## account_test
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 41
## account_update_tax_tags
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 16
  · [override] Company → 'Bolag'
## analytic
- override: 4
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 168
  · [override] Company → 'Bolag'
  · [override] Select a company for which the analytic distribution will be → 'Välj ett bolag för vilket den objektdist'
  · [override] You can't change the company of an analytic account that alr → 'Du kan inte ändra bolaget för ett objekt'
  · [override] You defined a distribution with analytic account(s) belongin → 'Du har angivit en fördelning med objektk'
## attachment_indexation
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## auth_ldap
- override: 3
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 35
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company LDAP configuration → 'Bolagets LDAP-konfiguration'
## auth_oauth
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 48
## auth_passkey
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 40
## auth_passkey_portal
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 11
## auth_password_policy
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 16
## auth_password_policy_portal
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## auth_password_policy_signup
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## auth_signup
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 100
  · [override] <table border="0" cellpadding="0" cellspacing="0" style="pad → '<table border="0" cellpadding="0" cellsp'
## auth_timeout
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 57
## auth_totp
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 80
## auth_totp_mail
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 44
## auth_totp_portal
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 16
## barcodes
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 58
  · [override] Companies → 'Bolag'
## barcodes_gs1_nomenclature
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 41
## base
- override: 172
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 7213
  · [override] 

Austrian charts of accounts (Einheitskontenrahmen 2010).
= → 'Österrikiska kontoplaner (Einheitskonten'
  · [override] 

Note: this might be a multi-company issue. Switching compa → 'Obs: detta kan vara ett problem med fler'
  · [override] 

Overview:
---------

* Chart of Accounts and Taxes templat → 'Översikt:\n---------\n\n* Mall för kontopla'
  · [override] 

This seems to be a multi-company issue, but you do not hav → 'Detta verkar vara ett problem med flera '
  · [override] 

This seems to be a multi-company issue, you might be able  → 'Det här verkar vara ett problem som gäll'
  · [override] 
Add a new model named "Identification Type" that extend the → 'Lägg till en ny modell med namnet "Ident'
  · [override] 
Add an extra tab in the checkout process of the website/eCo → 'Lägg till en extra flik i utcheckningspr'
  · [override] 
Allow Pine Labs POS payments
============================== → 'Tillåt Pine Labs POS-betalningar\n======='
  · [override] 
Assets management
=================
Manage assets owned by  → 'Förvaltning av tillgångar\n=============='
  · [override] 
Auto-complete partner companies' data
 → 'Automatisk komplettering av partnerbolag'
  · [override] 
Bridge module to integrate leaves in timesheet
============ → 'Bridge-modul för att integrera ledighete'
  · [override] 
Chart of Account for Venezuela.
=========================== → 'Kontoplan för Venezuela.\n==============='
  · [override] 
Czech accounting chart and localization.  With Chart of Acc → 'Tjeckisk kontoplan och lokalisering. Med'
  · [override] 
Documento di Trasporto (DDT)

Whenever goods are transferre → 'Trasportdokument (DDT)\n\nNär varor överfö'
  · [override] 
EU One Stop Shop (OSS) VAT
==========================

From → 'EU One Stop Shop (OSS) moms\n============'
  · [override] 
Egypt Accounting Module
=================================== → 'Egypten Redovisningsmodul\n=============='
  · [override] 
Electronic Data Interchange
=============================== → 'Elektroniskt datautbyte\n================'
  · [override] 
Electronic invoicing module
===========================

Al → 'Modul för elektronisk fakturering\n======'
  · [override] 
Functional
----------

In some Latinamerica countries, incl → 'Funktionell\n----------\n\nI vissa länder i'
  · [override] 
Functional
----------

This module add accounting features  → 'Funktionell\n----------\n\nDen här modulen '
  · [override] 
Functional
----------

This module adds accounting features → 'Funktionell\n----------\n\nDenna modul lägg'
  · [override] 
General Chart of Accounts.
==========================

This → 'Allmän kontoplan.\n======================'
  · [override] 
Generic Payroll system Integrated with Accounting.
======== → 'Generiskt lönesystem integrerat med redo'
  · [override] 
Indian - E-invoicing
====================
To submit invoici → 'Indian - E-fakturering\n================='
  · [override] 
Indian - E-waybill
====================================
To  → 'Indian - E-waybill\n====================='
  · [override] 
Module to automate letters for unpaid invoices, with multi- → 'Modul för att automatisera brev för obet'
  · [override] 
Oman Accounting Module
==================================== → 'Oman Redovisningsmodul\n================='
  · [override] 
Rwandan localisation containing:
- COA
- Taxes
- Tax report → 'Rwandisk lokalisering innehållande:\n- CO'
  · [override] 
Saudi Arabia Accounting Module
============================ → 'Saudiarabiens redovisningsmodul\n========'
  · [override] 
Saudi Arabia Payroll and End of Service rules.
============ → 'Saudiarabiens regler för löner och avslu'
  · [override] 
Send your shippings through bpost and track them online
=== → 'Skicka dina försändelser via bpost och s'
  · [override] 
Show your company address/partner address on Google Maps. C → 'Visa din bolagsadress/partneradress på G'
  · [override] 
Singapore accounting chart and localization.
============== → 'Singapore räkenskapsschema och lokaliser'
  · [override] 
Spanish charts of accounts (PGCE 2008).
=================== → 'Spanska kontoplaner (PGCE 2008).\n======='
  · [override] 
Tanzanian localisation containing:
- COA
- Taxes
- Tax repo → 'Tanzaniansk lokalisering innehållande:\n-'
  · [override] 
The base module to manage lunch.
========================== → 'Basmodul för hantering av lunch.\n======='
  · [override] 
This adds support for BBAN, Plusgiro & Bankgiro for swedish → 'Detta lägger till stöd för BBAN, Plusgir'
  · [override] 
This is the base module to manage the accounting chart for  → 'Detta är basmodulen för att hantera bokf'
  · [override] 
This is the base module to manage the accounting chart for  → 'Detta är basmodulen för att hantera kont'
  · [override] 
This is the base module to manage the accounting chart for  → 'Detta är basmodulen för att hantera bokf'
  · [override] 
This is the base module to manage the accounting chart for  → 'Detta är basmodulen för att hantera kont'
  · [override] 
This is the base module to manage the accounting chart for  → 'Detta är basmodulen för att hantera kont'
  · [override] 
This is the module to manage the Accounting Chart, VAT stru → 'Detta är modulen för att hantera kontopl'
  · [override] 
This is the module to manage the Canadian accounting chart  → 'Detta är modulen för att hantera den kan'
  · [override] 
This is the module to manage the accounting chart for Alger → 'Detta är modulen för att hantera kontopl'
  · [override] 
This is the module to manage the accounting chart for Franc → 'Detta är modulen för att hantera redovis'
  · [override] 
This is the module to manage the accounting chart, bank inf → 'Detta är modulen för att hantera kontopl'
  · [override] 
This module adds generic features to register an Odoo DB on → 'Denna modul lägger till generiska funkti'
  · [override] 
This module allows to print your payments on pre-printed ch → 'Med denna modul kan du skriva ut dina be'
  · [override] 
This module allows to print your payments on pre-printed ch → 'Med den här modulen kan du skriva ut din'
  · [override] 
This module allows to print your payments on pre-printed ch → 'Med denna modul kan du skriva ut dina be'
  · [override] 
This module enables generating the D.406 declaration from w → 'Denna modul gör det möjligt att generera'
  · [override] 
This module enables the generation of SEPA Direct Debit (SD → 'Denna modul möjliggör generering av SEPA'
  · [override] 
This module implements the accounting chart for OHADA area. → 'Denna modul implementerar kontoplanen fö'
  · [override] 
This module integrate Odoo so you can issue electronic docu → 'Denna modul integrerar Odoo så att du ka'
  · [override] 
This module sends the taxes information (mostly VAT) of the → 'Denna modul skickar skatteinformationen '
  · [override] 
United Arab Emirates Accounting Module
==================== → 'Förenade Arabemiraten Redovisningsmodul\n'
  · [override] 
VAT validation for Partner's VAT numbers.
================= → 'Momsvalidering för partners momsnummer.\n'
  · [override] 3PL Logistic Company → '3PL logistikbolag'
  · [override] <i class="fa fa-fw me-1 fa-building text-primary" title="Com → '<i class="fa fa-fw me-1 fa-building text'
  · [override] Access to unauthorized or invalid companies. → 'Tillgång för obehöriga eller ogiltiga bo'
  · [override] Accounting chart for Netherlands
=========================== → 'Redovisningsplan för Nederländerna\n====='
  · [override] Adds support for authentication by LDAP server.
============ → 'Lägger till stöd för autentisering via L'
  · [override] Auto-complete partner companies' data → 'Automatisk komplettering av uppgifter om'
  · [override] Calculate and report your company's Environmental, Social, a → 'Beräkna och rapportera ditt bolags påver'
  · [override] Check if the contact is a company, otherwise it is a person → 'Markera denna ruta om kontakten är ett b'
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Logo → 'Bolagslogo'
  · [override] Company Name → 'Bolagsnamn'
  · [override] Company Name Entity → 'Bolagets namn'
  · [override] Company Registry Placeholder → 'Platshållare för bolagsregister'
  · [override] Company Tagline → 'Bolagets slogan'
  · [override] Company tagline, which is included in a printed document's h → 'Bolagets slogan, som inkluderas i ett tr'
  · [override] Company used for the original currency (only used for t-esc) → 'Bolag som används för den ursprungliga v'
  · [override] Create a new company → 'Skapa ett nytt bolag'
  · [override] Create and manage company expense cards via Stripe → 'Skapa och hantera bolagets utläggskort v'
  · [override] Create and manage company expense cards via Stripe: Demo → 'Skapa och hantera bolagets utläggskort v'
  · [override] Create and manage the companies that will be managed by Odoo → 'Skapa och hantera de bolag som ska hante'
  · [override] Default Company → 'Standardbolag'
  · [override] Determines where the customer/company name should be placed, → 'Bestämmer var kundens/bolagets namn ska '
  · [override] Duplicating a company is not allowed. Please create a new co → 'Det är inte tillåtet att duplicera ett b'
  · [override] If set, action binding only applies for this company → 'Om angiven knyts åtgärden bara till dett'
  · [override] Inter Company Module for Sale/Purchase Orders and Invoices → 'Modul för försäljning/inköpsorder och fa'
  · [override] Inverse Company Rate → 'Omvänd bolagsränta'
  · [override] Is Company → 'Är bolag'
  · [override] Is Company Details Empty → 'Is Bolagsuppgifter Tom'
  · [override] Is Current Company Currency → 'Är bolagets nuvarande valuta'
  · [override] Is a Company → 'Är ett bolag'
  · [override] Module for synchronization of Documents between several comp → 'Modul för synkronisering av dokument mel'
  · [override] Module for synchronization of Documents between several comp → 'Modul för synkronisering av dokument mel'
  · [override] Module for synchronization of Inventory Documents between se → 'Modul för synkronisering av lagerdokumen'
  · [override] Multi Companies → 'Flera bolag'
  · [override] My Company (San Francisco) undertakes to do its best to supp → 'Mitt bolag (San Francisco) åtar sig att '
  · [override] Nicole works in IT sector <b>since 20 years</b>. She
        → 'Nicole har arbetat inom IT-sektorn i <b>'
  · [override] Number of Companies → 'Antal bolag'
  · [override] Odoo CRM
--------

Boost sales productivity, improve win rat → 'Odoo CRM\n--------\n\nÖka försäljningsprodu'
  · [override] Odoo Mass Mailing
-----------------

Easily send mass mailin → 'Odoo Massutskick\n-----------------\n\nSkic'
  · [override] Odoo Point of Sale
-----------------------------

Odoo's <a  → 'Odoo Point of Sale\n---------------------'
  · [override] Our experts invent, imagine and develop solutions which meet → 'Våra experter uppfinner, föreställer sig'
  · [override] Partner with same Company Registry → 'Partner med samma bolagsregister'
  · [override] Related Company → 'Relaterat bolag'
  · [override] Show your company address on Google Maps → 'Visa bolagets adress på Google Maps'
  · [override] Skipping a company check for model %(model_name)s. Its field → 'Hoppar över en bolagskontroll för modell'
  · [override] Social media connectors for company settings. → 'Anslutningar till sociala medier för bol'
  · [override] The company %(company_name)s cannot be archived because it i → 'Bolaget %(company_name)s kan inte arkive'
  · [override] The company assigned to this partner does not match the comp → 'Det bolag som tilldelats denna partner s'
  · [override] The company hierarchy cannot be changed. → 'Bolagshierarkin kan inte ändras.'
  · [override] The company name must be unique! → 'Bolagsnamnet måste vara unikt!'
  · [override] The default company for this user. → 'Standardbolaget för den här användaren.'
  · [override] The registry number of the company. Use it if it is differen → 'Bolagets registreringsnummer. Använd det'
  · [override] The selected company is not compatible with the companies of → 'Det valda bolaget är inte kompatibelt me'
  · [override] This currency is set on a company and therefore cannot be de → 'Denna valuta är inställd på ett bolag oc'
  · [override] This is your company's currency. → 'Detta är bolagets valuta.'
  · [override] Used to order Companies in the company switcher → 'Används för att beställa bolag i bolagsv'
  · [override] We are a technology consulting &amp; strategic advisory firm → 'Vi är ett konsultbolag inom teknik och s'
  · [override] Whether values for this field is company dependent → 'Huruvida värden för detta fält är bolags'
  · [override] e.g. My Company → 't.ex. Mitt Bolag'
  · [override] 
Manage time off requests and allocations
================== → 'Hantera ledighetsförfrågningar\n========='
  · [override] Deco Addict integrates ERP for Global Companies and supports → 'Deco Addict integrerar ERP för globala b'
  · [override] Odoo Human Resources
--------------------

With Odoo <a href → 'Odoo Human Resources\n-------------------'
  · [override] 
Functional
----------

In some Latinamerica countries, incl → 'Funktionell\n----------\n\nI vissa länder i'
  · [override] 

Austrian charts of accounts (Einheitskontenrahmen 2010).
= → 'Österrikiska kontoplaner (Einheitskonten'
  · [override] 

Overview:
---------

* Chart of Accounts and Taxes templat → 'Översikt:\n---------\n\n* Mall för kontopla'
  · [override] 
        This module enables the generation of SEPA Direct D → 'Denna modul gör det möjligt att generera'
  · [override] 
       Auto-complete partner companies' data
     → 'Automatisk komplettering av uppgifter om'
  · [override] 
Add all information on the employee form to manage contract → 'Lägg till all information i formuläret f'
  · [override] 
Assets management
=================
Manage assets owned by  → 'Hantering av tillgångar\n================'
  · [override] 
Australian Accounting Module
============================

 → 'Australisk redovisningsmodul\n==========='
  · [override] 
Base module for the Brazilian localization
================ → 'Basmodul för den brasilianska lokaliseri'
  · [override] 
Bridge module to integrate leaves in timesheet
============ → 'Bryggmodul för att integrera ledighet i '
  · [override] 
Czech accounting chart and localization.  With Chart of Acc → 'Tjeckisk bokföringsschema och lokaliseri'
  · [override] 
EU One Stop Shop (OSS) VAT
==========================

From → 'EU:s enda kontaktpunkt (OSS) Moms\n======'
  · [override] 
Electronic Data Interchange
=============================== → 'Elektroniskt datautbyte\n================'
  · [override] 
Functional
----------

This module add accounting features  → 'Funktionell\n----------\n\nDen här modulen '
  · [override] 
Generic Payroll system Integrated with Accounting.
======== → 'Allmänt lönesystem integrerat med bokför'
  · [override] 
Manage expenses by Employees
============================

 → 'Hantera utlägg per anställd\n============'
  · [override] 
Module to import OFX bank statements.
===================== → 'Modul för att importera OFX-bankutdrag.\n'
  · [override] 
Send your shippings through bpost and track them online
=== → 'Skicka dina försändelser via bpost och s'
  · [override] 
Show your company address/partner address on Google Maps. C → 'Visa ditt bolags adress/partneradress på'
  · [override] 
Singapore accounting chart and localization.
============== → 'Bokföringsschema och lokalisering i Sing'
  · [override] 
Spanish charts of accounts (PGCE 2008).
=================== → 'Spanska kontoplaner (PGCE 2008).\n======='
  · [override] 
The base module to manage lunch.
========================== → 'Grundmodulen för hantering av lunch.\n==='
  · [override] 
This is the latest basic Israelian localisation necessary t → 'Detta är den senaste grundläggande israe'
  · [override] 
This is the module to manage the Accounting Chart, VAT stru → 'Detta är modulen för att hantera kontopl'
  · [override] 
This is the module to manage the Canadian accounting chart  → 'Det här är modulen för att hantera kanad'
  · [override] 
This is the module to manage the accounting chart for Franc → 'Det här är modulen för att hantera konto'
  · [override] 
This is the module to manage the accounting chart for Vietn → 'Det här är modulen för att hantera konto'
  · [override] 
This module allows to print your payments on pre-printed ch → 'Med denna modul kan du skriva ut dina be'
  · [override] 
VAT validation for Partner's VAT numbers.
================= → 'Validering av moms för partnerns momsnum'
  · [override] 
WMS Accounting module
======================
This module ma → 'WMS Redovisningsmodul\n=================='
  · [override]  Module for synchronization of Documents between several com → 'Modul för synkronisering av dokument mel'
  · [override]  Module for synchronization of Documents between several com → 'Modul för synkronisering av dokument mel'
  · [override] - %(record)r belongs to company %(company)r and %(field)r (% → '- %(record)r tillhör bolaget %(company)r'
  · [override] - Record is company %(company)r and %(field)r (%(fname)s: %( → '- Post är bolaget %(company)r och %(fiel'
  · [override] Account Consolidation Demo Data using a Belgium and a US com → 'Demodata för konsolidering av konton med'
  · [override] Allowed Companies → 'Tillåtna bolag'
  · [override] Changing the company of a contact should only be done if it  → 'Du bör endast ändra en kontakts bolag om'
  · [override] Companies count → 'Antal bolag'
  · [override] Company Data → 'Bolagets data'
  · [override] Company Properties → 'Bolagsuppgifter'
  · [override] Company Property → 'Bolagsuppgifter'
  · [override] Create company → 'Skapa bolag'
  · [override] Incompatible companies on records: → 'Inkompatibla bolag i registren:'
  · [override] Note: this might be a multi-company issue. → 'Observera: Detta kan vara ett problem fö'
  · [override] Odoo CRM
--------

Boost sales productivity, improve win rat → 'Odoo CRM\n--------\n\nÖka försäljningsprodu'
  · [override] Odoo Mass Mailing
-----------------

Easily send mass mailin → 'Odoo massutskick\n-----------------\n\nSkic'
  · [override] Set your company data → 'Ange din bolagsinformation'
  · [override] Set your company's data for documents header/footer. → 'Ställ in ditt bolags uppgifter för sidhu'
  · [override] State of the onboarding company step → 'Status för bolaget för ombordstigning st'
  · [override] You cannot create recursive companies. → 'Du kan inte skapa rekursiva bolag.'
  · [override] You cannot reduce the number of decimal places of a currency → 'Du kan inte minska antalet decimaler i e'
## base_address_extended
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 30
## base_automation
- override: 0
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 172
## base_geolocalize
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 37
## base_iban
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 10
## base_import
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 158
## base_import_module
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 57
## base_install_request
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 29
## base_setup
- override: 13
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 99
  · [override] <span class="o_form_label" invisible="company_count &gt; 1"> → '<span class="o_form_label" invisible="co'
  · [override] Automatically enrich your contact base with company data → 'Berika automatiskt din kontaktbas med bo'
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Country Code → 'Bolagets landskod'
  · [override] Company Informations → 'Information om bolaget'
  · [override] Company Name → 'Bolagsnamn'
  · [override] Create corresponding in/out orders and bills when you sell/b → 'Skapa motsvarande in-/utorder och faktur'
  · [override] Inter-Company Transactions → 'Transaktioner mellan bolag'
  · [override] Is Root Company → 'Bolaget Is Root'
  · [override] Manage Companies → 'Hantera bolag'
  · [override] Number of Companies → 'Antal bolag'
  · [override] When populating your address book, Odoo provides a list of m → 'När du fyller i din adressbok ger Odoo e'
## base_sparse_field
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 26
## base_vat
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 53
  · [override] Companies → 'Bolag'
## board
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 19
## bus
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 36
## calendar
- override: 0
- converged 🎉: 23
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 415
## calendar_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 17
## certificate
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 86
  · [override] Company → 'Bolag'
## cloud_storage
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 23
## cloud_storage_azure
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 16
## cloud_storage_google
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 14
## cloud_storage_migration
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 47
## contacts
- override: 0
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 17
## crm
- override: 145
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 611
  · [override] # Leads/Opps assigned this month → '# kundämnen/möjligheter denna månad'
  · [override] # Unassigned Leads → '# otilldelade kundämnen'
  · [override] %(assigned)s leads allocated among %(team_count)s teams. → '%(assigned)s kundämnen fördelade mellan '
  · [override] %(assigned)s leads allocated to %(team_name)s team. → '%(assigned)s kundämnen tilldelade till %'
  · [override] %(attach_name)s (from %(lead_name)s) → '%(attach_name)s (från %(kundämne_name)s)'
  · [override] %(duplicates)s duplicates leads have been merged. → '%(duplicates)s duplicerade kundämnen har'
  · [override] %(members_assigned)s leads assigned among %(member_count)s s → '%(members_assigned)s kundämnen fördelade'
  · [override] <b>Write a few letters</b> to look for a company, or create  → '<b>Skriv ett par tecken</b> för att söka'
  · [override] <i class="fa fa-building" title="Company"/> → '<i class="fa fa-building" title="Bolag"/'
  · [override] <i class="fa fa-info-circle me-2" title="Assigned Lead Count → '<i class="fa fa-info-circle me-2" title='
  · [override] <span class="o_stat_text" invisible="duplicate_lead_count &l → '<span class="o_stat_text" invisible="dup'
  · [override] <span class="o_stat_text" invisible="not use_leads">Leads</s → '<span class="o_stat_text" invisible="not'
  · [override] <span class="o_stat_text"> Leads</span> → '<span class="o_stat_text">Kundämnen</spa'
  · [override] <span> leads assigned the last 30 days out of a maximum of < → '<span> kundämnen tilldelade de senaste 3'
  · [override] <span> leads assigned this month
                            → '<span> kundämnen tilldelade denna månad\n'
  · [override] <span>Merged the Lead/Opportunity</span> → '<span>Sammanslagning av kundämne/möjligh'
  · [override] <span>Periodically assign leads based on priorities and filt → '<span>Periodisk tilldelning av kundämnen'
  · [override] A lead in a Won stage cannot be lost. Move it to another sta → 'En kundämne i ett Won-stadium kan inte f'
  · [override] A new lead has been created and is not assigned to any team. → 'En ny kundämne har skapats och är inte t'
  · [override] A new lead has been created for the team "%(team_name)s". → 'En ny kundämne har skapats för teamet "%'
  · [override] Active Leads → 'Aktiva kundämnen'
  · [override] Activities marked as Done on Leads will appear here, providi → 'Aktiviteter som markerats som utförda på'
  · [override] Assign Leads → 'Tilldela kundämnen'
  · [override] Associated Lead → 'Associerad kundämne'
  · [override] Average Leads Capacity (on 30 days) → 'Kundämnekapacitet i snitt (på 30 dagar)'
  · [override] CRM: Lead Assignment → 'CRM: Tilldelning av kundämnen'
  · [override] Check the Stage Tracker of Leads to identify bottlenecks in  → 'Kontrollera etapperna för kundämnen för '
  · [override] Check this box to filter and qualify incoming requests as le → 'Bocka i rutan för att filtrera och kvali'
  · [override] Classify and analyze your lead/opportunity categories like:  → 'Klassificera och analysera dina kategori'
  · [override] Closed/Dead leads cannot be converted into opportunities. → 'Avslutade/döda kundämnen kan inte omvand'
  · [override] Company → 'Bolag'
  · [override] Company Information → 'Information om bolaget'
  · [override] Company Name → 'Bolagsnamn'
  · [override] Company Name: → 'Bolagsnamn:'
  · [override] Company: → 'Bolag:'
  · [override] Consider leads created as of the: → 'Överväg kundämnen som skapade från och m'
  · [override] Conversion Date from Lead to Opportunity → 'Konverteringsdatum från kundämne till mö'
  · [override] Convert Lead to Opportunity (in mass) → 'Konvertera kundämnen till möjligheter (i'
  · [override] Convert Lead to Opportunity (not in mass) → 'Konvertera kundämnen till möjlighet (ej '
  · [override] Convert visitors of your website into leads and perform data → 'Konvertera besökare på din webbplats til'
  · [override] Create Leads/Opportunities from your website's traffic → 'Skapa kundämnen/möjligheter från webbpla'
  · [override] Create a Lead → 'Skapa ett kundämne'
  · [override] Create a Lead Activity Plan → 'Skapa en aktivitetsplan för kundämnen'
  · [override] Create a lead mining request directly from the opportunity p → 'Skapa en kundämnegenerering direkt från '
  · [override] Create a new lead → 'Skapa nytt kundämne'
  · [override] Customer Company → 'Kundbolag'
  · [override] Did you know you can search a company by name or VAT number  → 'Visste du att du kan söka efter ett bola'
  · [override] Don't link to a company → 'Länka inte till ett bolag'
  · [override] Enrich all leads automatically → 'Berika alla kundämnen automatiskt'
  · [override] Enrich lead automatically → 'Berika kundämnen automatiskt'
  · [override] Enrich leads on demand only → 'Berika kundämnen endast på begäran'
  · [override] Enrich your leads automatically with company data based on t → 'Berika dina kundämnen automatiskt med bo'
  · [override] Enrich your leads with company data based on their email add → 'Berika dina kundämnen med bolagsdata bas'
  · [override] Exceed monthly lead assignement → 'Överskrid den månatliga kundämnentilldel'
  · [override] Fields that can be used for predictive lead scoring computat → 'Fält som kan användas för beräkning av f'
  · [override] For a sales team, there is nothing worse than being dry on l → 'För ett säljteam finns det inget värre ä'
  · [override] Generate new leads based on their country, industries, size, → 'Generera nya kundämnen baserat på land, '
  · [override] Generate new leads based on their country, industry, size, e → 'Generera nya kundämnen baserat på land, '
  · [override] Import Template for Leads & Opportunities → 'Importmall för kundämnen och möjligheter'
  · [override] Kpi Crm Lead Created Value → 'KPI-värde i CRM för kundämne'
  · [override] Lead → 'Kundämne'
  · [override] Lead Assign → 'Tilldela kundämne'
  · [override] Lead Assignment Filter → 'Filter för tilldelning av kundämnen'
  · [override] Lead Assignment requested by %(user_name)s → 'Tilldelning av kundämne begärd av %(user'
  · [override] Lead Average Capacity → 'Genomsnittlig kapacitet för kundämne'
  · [override] Lead Enrichment → 'Berika kundämnen'
  · [override] Lead Generation → 'Kundämnegenerering'
  · [override] Lead Mining → 'Kundämnegenerering'
  · [override] Lead Properties → 'Egenskaper för kundämne'
  · [override] Lead Scoring Frequency → 'Frekvens för poängsättning av kundämne'
  · [override] Lead Scoring Frequency Fields → 'Fält för frekvens för poängsättning av k'
  · [override] Lead Scoring Frequency Fields in String → 'Fält för frekvens för poängsättning av k'
  · [override] Lead Scoring Starting Date → 'Startdatum för poängsättning av kundämne'
  · [override] Lead Scoring Starting Date in String → 'Startdatum för poängsättning av kundämne'
  · [override] Lead Tags → 'Kundämne taggar'
  · [override] Lead or Opportunity → 'Kundämne eller möjlighet'
  · [override] Lead/Opportunities automatic assignment is limited to manage → 'Automatisk tilldelning av kundämnen/möjl'
  · [override] Lead/Opportunity created → 'Kundämne/möjlighet skapad'
  · [override] Leads → 'Kundämnen'
  · [override] Leads (30 days) → 'Kundämnen (30 dagar)'
  · [override] Leads (last 24h) → 'Kundämnen (senaste 24 timmarna)'
  · [override] Leads Analysis → 'Kundämnesanalys'
  · [override] Leads Analysis allows you to check different CRM related inf → 'Kundämnesanalys låter dig kontrollera ol'
  · [override] Leads Assigned → 'Tilldelade kundämnen'
  · [override] Leads Count → 'Antal kundämnen'
  · [override] Leads Generation → 'Kundämnengeneration'
  · [override] Leads are the qualification step before the creation of an o → 'Kundämnen är kvalificeringssteget före s'
  · [override] Leads or Opportunities → 'Kundämnen eller möjligheter'
  · [override] Leads that are assigned to me → 'Kundämnen tilldelade mig'
  · [override] Leads that are not assigned → 'Otilldelade kundämnen'
  · [override] Leads that you selected that have duplicates. If the list is → 'Kundämnen som du valt som har dubbletter'
  · [override] Leads with existing duplicates (for information) → 'Kundämnen med befintliga dubbletter (för'
  · [override] Leads/Opportunities → 'Kundämnen/Möjligheter'
  · [override] Leads/Opportunities count → 'Antal kundämnen/möjligheter'
  · [override] Linked partner (optional). Usually created when converting t → 'Länkad kontakt (valfri). Oftast skapad n'
  · [override] Lost Lead → 'Förlorad kundämne'
  · [override] Manage members or partners. Members get grades and pricelist → 'Hantera medlemmar eller partners. Medlem'
  · [override] Merge Leads/Opportunities → 'Sammanfoga kundämnen/möjligheter'
  · [override] Merge with existing leads/opportunities of each partner → 'Sammanfoga med befintliga kundämnen/möjl'
  · [override] Monthly average leads capacity for all salesmen belonging to → 'Månatlig genomsnittlig kapacitet för kun'
  · [override] My Leads → 'Mina kundämnen'
  · [override] New Lead → 'Ny kundämne'
  · [override] New Leads → 'Nya kundämnen'
  · [override] No allocated leads to %(team_name)s team and its salesperson → 'Inga tilldelade kundämnen till teamet %('
  · [override] No allocated leads to %(team_name)s team because it has no c → 'Inga allokerade kundämnen till teamet %('
  · [override] No allocated leads to any team or salesperson. Check your Sa → 'Inga tilldelade kundämnen till något tea'
  · [override] No lead assigned to salespersons because no unassigned lead  → 'Inga kundämnen tilldelade till säljare e'
  · [override] No new lead allocated to %(team_name)s team because no unass → 'Inga nya kundämnen tilldelade till teame'
  · [override] No new lead allocated to the teams because no lead match the → 'Inga nya kundämnen har tilldelats teamen'
  · [override] Number of leads and opportunities assigned this last month. → 'Antal kundämnen och möjligheter tilldela'
  · [override] Number of leads assigned to this member in the last 24 hours → 'Antal kundämnen som tilldelats denna med'
  · [override] Number of leads assigned to this member in the last 30 days → 'Antal kundämnen som tilldelats denna med'
  · [override] Potential Duplicate Lead → 'Potentiell dubblett-kundämne'
  · [override] Potential Duplicate Lead Count → 'Antal potentiella dubblett-kundämne'
  · [override] Predictive Lead Scoring → 'Förutspådd poängsättning av kundämnen'
  · [override] Predictive Lead Scoring Field Labels → 'Förutspådd poängsättning av kundämnen fä'
  · [override] Predictive Lead Scoring: Recompute Automated Probabilities → 'Förutspådd poängsättning av kundämnen: o'
  · [override] Search Leads → 'Sök kundämnen'
  · [override] Select at least two Leads/Opportunities from the list to mer → 'Välj minst två kundämnen/möjligheter frå'
  · [override] Show Lead Menu → 'Visa menyn för kundämnen'
  · [override] Show only lead → 'Visa bara kundämnen'
  · [override] Show only leads → 'Visa bara kundämnen'
  · [override] The contacts below have been added as followers of this lead → 'Kontakterna nedan har lagts till som föl'
  · [override] The email address associated with this channel. New emails r → 'E-postadressen som är kopplad till denna'
  · [override] The name of the future partner company that will be created  → 'Namnet på kontaktens bolag som kommer at'
  · [override] This analysis shows you how many leads have been created per → 'Denna analys visar dig hur många kundämn'
  · [override] This can be used to automatically assign leads to sales pers → 'Detta kan användas för att automatiskt t'
  · [override] This can be used to compute statistical probability to close → 'Detta kan användas för att beräkna stati'
  · [override] This new lead created by %(creation_source)s was automatical → 'Denna nya kundämne som skapades av %(cre'
  · [override] This will assign leads to all members. Do you want to procee → 'Detta kommer att tilldela kundämnen till'
  · [override] Tip: Did you know Odoo has built-in lead mining? → 'Tips: visste du att Odoo har inbyggd kun'
  · [override] To prevent data loss, Leads and Opportunities can only be me → 'För att förhindra dataförlust kan kundäm'
  · [override] Type is used to separate Leads and Opportunities → 'Typ används för att separera kundämnen f'
  · [override] UX: Limit to lead company or all if no company → 'UX: begränsa till kundämnen bolag, eller'
  · [override] Unassigned Leads → 'Otilldelade kundämnen'
  · [override] Use Leads → 'Använd kundämnen'
  · [override] Use leads if you need a qualification step before creating a → 'Använd kundämnen om du behöver ett kvali'
  · [override] Use leads if you need a qualification step before creating a → 'Använd kundämnen om du behöver ett kvali'
  · [override] User Company → 'Bolag'
  · [override] Visits to Leads → 'Besökare till kundämnen'
  · [override] for the leads created as of the → 'för de kundämnen som skapats från och me'
  · [override] For a sales team, there is nothing worse than being dry on l → 'För ett säljteam finns inget värre än at'
  · [override] <span class="o_stat_text" attrs="{'invisible': [('use_leads' → '<span class="o_stat_text" attrs="{\'invis'
  · [override] Select Leads/Opportunities → 'Välj kundämnen/möjligheter'
  · [override] Unassigned Lead → 'Otilldelad kundämne'
## crm_iap_enrich
- override: 3
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 16
  · [override] <span> No company data found based on the email address or e → '<span>Ingen bolagsinformation hittades b'
  · [override] Enrich lead with company data → 'Berika lead med bolagsdata'
  · [override] Enrich opportunity with company data → 'Berika möjlighet med bolagsdata'
## crm_iap_mine
- override: 6
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 140
  · [override] %(credit_count)d credits will be consumed to find %(company_ → '%(credit_count)d krediter kommer att för'
  · [override] <span class="col-md-6">Extra contacts per Company</span> → '<span class="col-md-6">Extra kontakter p'
  · [override] Companies → 'Bolag'
  · [override] Companies and their Contacts → 'Bolag och deras kontaktpersoner'
  · [override] Company Size Max → 'Bolags Maxstorlek'
  · [override] Up to %(credit_count)d additional credits will be consumed t → 'Upp till %(credit_count)d ytterligare kr'
## crm_livechat
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 35
## crm_mail_plugin
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 19
## crm_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 2
## data_recycle
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 65
  · [override] Company → 'Bolag'
## delivery
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 189
  · [override] Amount of the order to benefit from a free shipping, express → 'Orderbeloppet för att dra nytta av grati'
  · [override] Company → 'Bolag'
## delivery_mondialrelay
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 19
## delivery_stock_picking_batch
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 10
## digest
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 96
  · [override] Company → 'Bolag'
## event
- override: 8
- converged 🎉: 4
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 687
  · [override] <i class="fa fa-building" title="Attendee Company"/> → '<i class="fa fa-building" title="Deltaga'
  · [override] <span class="o_event_badge_font_faded">My Placeholder Compan → '<span class="o_event_badge_font_faded">M'
  · [override] Check this for order-level questions (e.g., 'Company Name')  → 'Kontrollera detta för frågor på orderniv'
  · [override] Company → 'Bolag'
  · [override] Company Name → 'Bolagsnamn'
  · [override] <table border="0" cellpadding="0" cellspacing="0" style="pad → '<table border="0" cellpadding="0" cellsp'
  · [override] <table border="0" cellpadding="0" cellspacing="0" style="pad → '<table border="0" cellpadding="0" cellsp'
  · [override] <table border="0" cellpadding="0" cellspacing="0" style="pad → '<table border="0" cellpadding="0" cellsp'
## event_booth
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 121
## event_booth_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 66
## event_crm
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 94
  · [override] Company → 'Bolag'
  · [override] Restrict the trigger of this rule to events belonging to a s → 'Begränsa utlösningen av denna regel till'
## event_crm_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## event_product
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 27
## event_sale
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 134
  · [override] Company → 'Bolag'
## event_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 13
## fleet
- override: 1
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 480
  · [override] Company → 'Bolag'
## gamification
- override: 3
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 413
  · [override] Set your Company Data → 'Ange dina bolagsuppgifter'
  · [override] Set your Company Logo → 'Ange bolagets logotyp'
  · [override] The target and current value are defined in the company curr → 'Målet och det aktuella värdet definieras'
## gamification_sale_crm
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 17
## google_account
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## google_address_autocomplete
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 19
## google_calendar
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 73
## google_gmail
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 47
## google_recaptcha
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 22
## hr
- override: 16
- converged 🎉: 2
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 868
  · [override] <i class="fa fa-building-o" role="img" aria-label="Company"  → '<i class="fa fa-building-o" role="img" a'
  · [override] A user cannot be linked to multiple employees in the same co → 'En användare kan inte länkas till flera '
  · [override] As an employee of our company, you will <b>collaborate with  → 'Som anställd i vårt bolag kommer du att '
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Country → 'Bolagsland'
  · [override] Company Country Code → 'Bolagets landskod'
  · [override] Company Logo → 'Bolagslogo'
  · [override] Company Working Hours → 'Bolagets arbetstid'
  · [override] Company country → 'Bolagets land'
  · [override] Company employee → 'Bolagsanställd'
  · [override] Each employee has a chance to see the impact of his work.
   → 'Varje anställd har en chans att se effek'
  · [override] Real responsibilities and challenges in a fast evolving comp → 'Verkligt ansvar och utmaningar i ett sna'
  · [override] Set default company schedule to manage your employees workin → 'Ange standardschema för bolaget för att '
  · [override] The name of the job position must be unique per department i → 'Jobbets namn måste vara unikt per avdeln'
  · [override] To avoid multi company issues (losing the access to your pre → 'För att undvika problem med flera bolag '
## hr_attendance
- override: 5
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 410
  · [override] Allow a period of time (around working hours) where extra ti → 'Tillåt en tidsperiod (runt arbetstid) dä'
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Logo → 'Bolagslogo'
  · [override] Tolerance Time In Favor Of Company → 'Toleranstid till förmån för bolaget'
## hr_calendar
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 6
## hr_expense
- override: 12
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 476
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Payment method allowed for expenses paid by company. → 'Tillåten betalningsmetod för kostnader s'
  · [override] Payment methods available for expenses paid by company → 'Tillgängliga betalningsmetoder för kostn'
  · [override] Report Company Currency → 'Rapport bolagets valuta'
  · [override] Tax amount in company currency → 'Skattebelopp i bolagets valuta'
  · [override] The company's default journal used when an employee expense  → 'Bolagets standardjournal som används när'
  · [override] The payment method used when the expense is paid by the comp → 'Betalningsmetoden som används när utlägg'
  · [override] When the cost of an expense product is different than 0, the → 'När kostnaden för en utläggsprodukt skil'
  · [override] An expense report must contain only lines from the same comp → 'En utläggsrapport får endast innehålla r'
  · [override] Price in Company Currency → 'Pris i bolagets valuta'
  · [override] You can only generate accounting entry for approved expense( → 'Du kan bara skapa verifikat för godkända'
## hr_fleet
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 37
  · [override] Company Car → 'Bolagsbil'
  · [override] Release Company Car → 'Frigör bolagsbil'
## hr_gamification
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 53
## hr_holidays
- override: 5
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 972
  · [override] Allow to create requests in batchs:
- By Employee: for a spe → 'Tillåt att skapa förfrågningar i batcher'
  · [override] Company → 'Bolag'
  · [override] Employee Company → 'Anställda Bolag'
  · [override] There is no employee set on the time off. Please make sure y → 'Det finns ingen anställd som är inställd'
  · [override] e.g. Extra recuperation, Company unavailability, ... → 't.ex. extra återhämtning, bolaget är int'
## hr_holidays_attendance
- override: 0
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 68
## hr_holidays_homeworking
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## hr_homeworking
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 59
## hr_homeworking_calendar
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 24
## hr_hourly_cost
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 4
## hr_livechat
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 1
## hr_maintenance
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 23
## hr_org_chart
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 34
## hr_presence
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 39
  · [override] Companies → 'Bolag'
## hr_recruitment
- override: 3
- converged 🎉: 2
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 602
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Select the location where the applicant will work. Addresses → 'Välj den ort där den sökande ska arbeta.'
## hr_recruitment_skills
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 47
## hr_recruitment_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## hr_recruitment_survey
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 57
## hr_skills
- override: 3
- converged 🎉: 2
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 427
  · [override] Company → 'Bolag'
  · [override] Demo Company Name → 'Demo Bolagsnamn'
  · [override] Secretary, company → 'Sekreterare, bolag'
## hr_skills_event
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 9
## hr_skills_slides
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 33
## hr_skills_survey
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 19
## hr_timesheet
- override: 5
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 251
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] The Internal Project of a company should be in that company. → 'Ett bolags interna projekt bör ligga ino'
  · [override] The project, the task and the analytic accounts of the times → 'Projektet, uppgiften och de analytiska k'
  · [override] Timesheets must be created with an active employee in the se → 'Tidrapporter måste skapas med en aktiv a'
## hr_timesheet_attendance
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 22
  · [override] Company → 'Bolag'
## hr_work_entry
- override: 1
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 256
  · [override] Company → 'Bolag'
## hr_work_entry_holidays
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 11
## html_builder
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 532
## html_editor
- override: 0
- converged 🎉: 2
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 405
## http_routing
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 26
## iap
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 44
  · [override] Company → 'Bolag'
  · [override] Get quality leads and opportunities: convert your website vi → 'Få leads och möjligheter av hög kvalitet'
## iap_crm
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## iap_mail
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 40
  · [override] <i class="fa fa-fw me-2 fa-building text-primary"/>
         → '<i class="fa fa-fw me-2 fa-building text'
  · [override] Company → 'Bolag'
## im_livechat
- override: 0
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 622
## l10n_se
- override: 4
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 109
  · [override] Companies → 'Bolag'
  · [override] Liabilities to associated companies and jointly controlled c → 'Skulder till intressebolag och gemensamt'
  · [override] Liabilities to group companies → 'Skulder till bolagsgrupper'
  · [override] Liabilities to other companies in which there is an ownershi → 'Skulder till övriga bolag som det finns '
## link_tracker
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 59
## loyalty
- override: 1
- converged 🎉: 2
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 360
  · [override] Company → 'Bolag'
## lunch
- override: 4
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 381
  · [override] A cashmove can either be an expense or a payment.<br>
       → 'En kassaförflyttning kan antingen vara e'
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Here you can see your cash moves.<br>A cash move can either  → 'Här kan du se dina kontantrörelser.<br>E'
## mail
- override: 8
- converged 🎉: 21
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 2217
  · [override] Companies → 'Bolag'
  · [override] Companies using this domain as default for sending mails → 'Bolag som använder denna domän som stand'
  · [override] Company → 'Bolag'
  · [override] Related Company → 'Relaterat bolag'
  · [override] The records must belong to the same company. → 'Posterna måste tillhöra samma bolag.'
  · [override] We could not create alias %(alias_name)s because domain %(al → 'Vi kunde inte skapa alias %(alias_name)s'
  · [override] We could not create alias %(alias_name)s because domain %(al → 'Vi kunde inte skapa alias %(alias_name)s'
  · [override] You cannot delete those groups, as the Whole Company group i → 'Du kan inte ta bort dessa grupper, efter'
## mail_bot
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 46
## mail_group
- override: 1
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 266
  · [override] My Company News → 'Nyheter om mitt bolag'
## mail_plugin
- override: 21
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 96
  · [override] Company → 'Bolag'
  · [override] Company Created. → 'Bolaget skapades.'
  · [override] Company Insights → 'Bolagsinsikter'
  · [override] Company Type → 'Bolagstyp'
  · [override] Company created → 'Skapat bolag'
  · [override] Company updated → 'Bolaget uppdaterat'
  · [override] Could not autocomplete the company. Internal error. Try agai → 'Det gick inte att autofullborda bolaget.'
  · [override] Create a Company → 'Skapa ett bolag'
  · [override] Create a company → 'Skapa ett bolag'
  · [override] Domain used to find the company → 'Domän som används för att hitta bolaget'
  · [override] Enrich Company → 'Utöka bolaget'
  · [override] No additional insights were found for this company → 'Inga ytterligare detaljer hittades för d'
  · [override] No company attached to this contact → 'Inget bolag kopplat till denna kontakt'
  · [override] No company attached to this contact. → 'Inget bolag kopplat till denna kontakt.'
  · [override] No company linked to this contact could be enriched → 'Inget bolag som är kopplat till denna ko'
  · [override] No company linked to this contact could be enriched or found → 'Inget bolag kopplat till denna kontakt k'
  · [override] No insights for this company. → 'Inga detaljer för detta bolag.'
  · [override] Save the contact to create the company → 'Spara kontakten för att skapa bolaget'
  · [override] The partner already has a company related to him → 'Partnern har redan ett bolag som är rela'
  · [override] This company has no email address and could not be enriched. → 'Detta bolag har ingen e-postadress och k'
  · [override] This contact has no email address, no company could be enric → 'Denna kontakt har ingen e-postadress, in'
## maintenance
- override: 1
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 298
  · [override] Company → 'Bolag'
## marketing_card
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 198
## mass_mailing
- override: 8
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 1173
  · [override] Best practices to digitize your company → 'Bästa praxis för att digitalisera ditt b'
  · [override] Companies → 'Bolag'
  · [override] Company Name → 'Bolagsnamn'
  · [override] Founder and chief visionary, Tony is the driving force behin → 'Tony är bolagets grundare och främsta vi'
  · [override] Iris, with her international experience, helps us easily und → 'Iris, med sin internationella erfarenhet'
  · [override] Mich loves taking on challenges. With his multi-year experie → 'Mich älskar att ta sig an utmaningar. Me'
  · [override] Want to import country, company name and more? → 'Vill du importera land, bolagsnamn och m'
  · [override] Infographic: Our Company Throughout The Generations → 'Infografik: Vårt bolag genom generatione'
## mass_mailing_crm
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 17
## mass_mailing_crm_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## mass_mailing_event
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 7
## mass_mailing_event_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## mass_mailing_event_track
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 7
## mass_mailing_event_track_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## mass_mailing_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 20
## mass_mailing_sale_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 6
## mass_mailing_slides
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## mass_mailing_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 189
## mass_mailing_themes
- override: 18
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 225
  · [override] . I put it on hold for 6 years, waiting for the right moment → '. Jag lade det på is i 6 år och väntade '
  · [override] After a few months of pitching investors, I got roughly 10 L → 'Efter några månaders pitchande av invest'
  · [override] As usual, I should have listened to my wife. She is way more → 'Som vanligt borde jag ha lyssnat på min '
  · [override] Being a Mature Company → 'Att vara ett moget bolag'
  · [override] Best practices to digitize your company → 'Bästa praxis för att digitalisera ditt b'
  · [override] Company News → 'Nyheter om bolaget'
  · [override] Discover Odoo, the all-in-one management platform, and its a → 'Upptäck Odoo, allt-i-ett-hanteringsplatt'
  · [override] I needed to change the world. I wanted to... You know how it → 'Jag behövde förändra världen. Jag ville.'
  · [override] I signed the LOI. I didn't realize that this contract could  → 'Jag skrev under avsiktsförklaringen. Jag'
  · [override] If you wish to evaluate Odoo while taking into account your  → 'Om du vill utvärdera Odoo och samtidigt '
  · [override] In May 2014, we renamed the company and product to Odoo. → 'I maj 2014 bytte vi namn på bolaget och '
  · [override] Infographic: Our Company Throughout The Generations → 'Infografik: Vårt bolag genom generatione'
  · [override] Making companies a better place, one app at a time → 'Att göra bolag till en bättre plats, en '
  · [override] The new financing will support the acceleration of the outst → 'Den nya finansieringen kommer att bidra '
  · [override] We wanted to switch from a service company to a software pub → 'Vi ville gå från att vara ett servicebol'
  · [override] With this money in our bank account, we boosted two departme → 'Med dessa pengar på vårt bankkonto först'
  · [override] You may then take part in round table discussions with exper → 'Du kan sedan delta i rundabordssamtal me'
  · [override] the fastest growing company of Belgium → 'det snabbast växande bolaget i Belgien'
## microsoft_account
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 9
## microsoft_calendar
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 72
## microsoft_outlook
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 46
## mrp
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 1178
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
## mrp_account
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 109
## mrp_landed_costs
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 6
## mrp_product_expiry
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 11
## mrp_repair
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 11
## mrp_subcontracting
- override: 4
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 112
  · [override] Companies → 'Bolag'
  · [override] In order to manage stock accurately, subcontracting location → 'För att kunna hantera lager på ett korre'
  · [override] You cannot alter the company's subcontracting location → 'Du kan inte ändra bolagets plats för und'
  · [override] Check this box to create a new dedicated subcontracting loca → 'Markera denna ruta för att skapa en ny s'
## mrp_subcontracting_account
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 6
## mrp_subcontracting_dropshipping
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 18
  · [override] Companies → 'Bolag'
  · [override] Check this box to create a new dedicated subcontracting loca → 'Markera denna ruta för att skapa en ny s'
## mrp_subcontracting_landed_costs
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## mrp_subcontracting_purchase
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 24
## onboarding
- override: 7
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 64
  · [override] All Onboarding Progress Records (across companies). → 'Alla register över framsteg inom onboard'
  · [override] All related Onboarding Progress Step Records (across compani → 'Alla relaterade poster för Onboarding Pr'
  · [override] Company → 'Bolag'
  · [override] Is per company → 'Är per bolag'
  · [override] Onboarding Progress Step for the current context (company). → 'Introduktionsförlopps steg för den aktue'
  · [override] Onboarding Progress for the current context (company). → 'Introduktions förlopp för det aktuella s'
  · [override] Should be done per company? → 'Bör detta göras per bolag?'
## partner_autocomplete
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 28
  · [override] Companies → 'Bolag'
  · [override] Unable to enrich company (no credit was consumed). → 'Kunde inte fylla i bolagsuppgifter (inge'
## partnership
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 43
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
## payment
- override: 5
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 631
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Please switch to company → 'Vänligen byt till bolag'
  · [override] The main currency of the company, used to display monetary f → 'Bolagets huvudvaluta, används för att vi'
  · [override] You cannot change the company of a payment provider with exi → 'Du kan inte byta bolag för en betalnings'
## payment_adyen
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 38
## payment_aps
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 21
## payment_asiapay
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 32
## payment_authorize
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 40
## payment_buckaroo
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 17
## payment_custom
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 25
## payment_demo
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 50
## payment_dpo
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 11
  · [override] Company Token → 'Bolagstoken'
  · [override] DPO Company Token → 'Bolagstoken hos DPO'
## payment_ecpay
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 17
## payment_flutterwave
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 24
## payment_iyzico
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 14
## payment_mercado_pago
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 59
## payment_mollie
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 16
## payment_nuvei
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 22
## payment_paymob
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 29
## payment_paypal
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 30
## payment_payu
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 23
## payment_razorpay
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 52
## payment_redsys
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 21
## payment_stripe
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 42
## payment_toss_payments
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 18
## payment_worldline
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 26
## payment_xendit
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 36
## phone_validation
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 69
## point_of_sale
- override: 19
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 1879
  · [override] All available pricelists must be in the same currency as the → 'Alla tillgängliga prislistor måste vara '
  · [override] All payment methods must be in the same currency as the Sale → 'Alla betalningsmetoder måste vara i samm'
  · [override] Choose a specific fiscal position at the order depending on  → 'Välj en specifik skatteområde vid bestäl'
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company has chart of accounts → 'Bolaget har en kontoplan'
  · [override] Conversion rate from company currency to order currency. → 'Omräkningskurs från bolagets valuta till'
  · [override] Ensure that there is an existing bank journal. Check if char → 'Se till att det finns en befintlig bankj'
  · [override] Is Using Company Currency → 'Använder bolagets valuta'
  · [override] Leave empty to use the default account from the company sett → 'Lämna tomt för att använda standardkonto'
  · [override] The default pricelist must belong to no company or the compa → 'Standardprislistan får inte tillhöra någ'
  · [override] The fiscal position used in the original order is not loaded → 'Den skatteområde som användes i den ursp'
  · [override] The invoice journal must be in the same currency as the Sale → 'Fakturadagboken måste vara i samma valut'
  · [override] The payment methods for the point of sale %s must belong to  → 'Betalningsmetoderna för försäljningsstäl'
  · [override] The selected pricelists must belong to no company or the com → 'De valda prislistorna får inte tillhöra '
  · [override] There is no Chart of Accounts configured on the company. Ple → 'Det finns ingen kontoplan konfigurerad p'
  · [override] Use fiscal positions to get different taxes by order → 'Använd skatteområden för att få olika sk'
  · [override] e.g. Company Address, Website → 't.ex. bolagsadress, webbplats'
  · [override] You cannot invoice orders belonging to different companies. → 'Du kan inte fakturera order som hör till'
## portal
- override: 3
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 254
  · [override] Company Name → 'Bolagsnamn'
  · [override] Multi company reports are not supported. → 'Rapporter för flera bolag stöds inte.'
  · [override] Changing company name is not allowed once document(s) have b → 'Det är inte tillåtet att ändra bolagsnam'
## portal_rating
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 37
## pos_account_tax_python
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## pos_adyen
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 27
  · [override] Terminal %(terminal)s is already used in company %(company)s → 'Terminal %(terminal)s används redan i bo'
## pos_cashdro
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 22
## pos_cashmatic
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 15
## pos_discount
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 21
## pos_dpopay
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 44
## pos_edi_ubl
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## pos_event
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 53
## pos_event_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## pos_glory_cash
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 65
## pos_hr
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 63
## pos_imin
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 9
## pos_loyalty
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 142
## pos_mercado_pago
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 29
## pos_mollie
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 13
## pos_mrp
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## pos_online_payment
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 89
  · [override] All payment providers configured for an online payment metho → 'Alla betalningsleverantörer som är konfi'
## pos_online_payment_self_order
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 21
## pos_pine_labs
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 31
## pos_qfpay
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 27
## pos_razorpay
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 39
## pos_repair
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## pos_restaurant
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 303
## pos_restaurant_adyen
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 7
## pos_safaricom
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 52
## pos_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 98
## pos_sale_loyalty
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## pos_sale_margin
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## pos_self_order
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 282
## pos_self_order_adyen
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## pos_self_order_pine_labs
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 4
## pos_self_order_qfpay
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 4
## pos_self_order_razorpay
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## pos_self_order_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 4
## pos_self_order_stripe
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## pos_self_order_viva_com
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## pos_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 10
## pos_stripe
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 21
  · [override] Stripe payment provider for company %s is missing → 'Stripe-betalningsleverantör för bolaget '
## pos_viva_com
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 37
## privacy_lookup
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 54
## product
- override: 3
- converged 🎉: 2
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 820
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Settings → 'Bolagsinställningar'
## product_email_template
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 10
## product_expiry
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 98
## product_margin
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 59
## product_matrix
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 14
  · [override] My Company Tshirt (GRID) → 'T-shirt med Mitt Bolag (RUTNÄT)'
  · [override] Show your company love around you =). → 'Visa din omgivning hur mycket du älskar '
## project
- override: 9
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 915
  · [override] Company → 'Bolag'
  · [override] Partner company cannot be different from its assigned projec → 'Partnerbolaget får inte vara ett annat ä'
  · [override] Partner company cannot be different from its assigned tasks' → 'Partnerbolaget får inte vara annorlunda '
  · [override] The project and the associated partner must be linked to the → 'Projektet och den associerade partnern m'
  · [override] The project's company cannot be changed if its analytic acco → 'Projektets bolag kan inte ändras om dess'
  · [override] The task and the associated partner must be linked to the sa → 'Uppgiften och den tillhörande partnern m'
  · [override] This project is associated with %(project_company)s, whereas → 'Det här projektet är associerat med %(pr'
  · [override] This project is not associated with any company, while the s → 'Det här projektet är inte knutet till nå'
  · [override] You are not able to switch the company of this stage to %(co → 'Du kan inte byta bolag för detta steg ti'
## project_account
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 7
## project_hr_expense
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## project_hr_skills
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 8
## project_mail_plugin
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 30
## project_mrp
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 12
## project_mrp_account
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 14
## project_mrp_stock_landed_costs
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## project_purchase
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 11
## project_purchase_stock
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 4
## project_sale_expense
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## project_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 12
## project_stock
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 7
## project_stock_account
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 15
## project_stock_landed_costs
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## project_timesheet_holidays
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 28
  · [override] Companies → 'Bolag'
## project_todo
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 102
## purchase
- override: 8
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 658
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Currency → 'Bolagets valuta'
  · [override] Default on whether the sales price used on the product and i → 'Standard om huruvida det försäljningspri'
  · [override] Put an address if you want to deliver directly from the vend → 'Ange en adress om du vill leverera direk'
  · [override] Technical field to filter the available taxes depending on t → 'Tekniskt fält för att filtrera de tillgä'
  · [override] This analysis allows you to easily check and analyse your co → 'Med denna analys kan du enkelt kontrolle'
  · [override] Your quotation contains products from company %(product_comp → 'Ditt erbjudande innehåller produkter frå'
## purchase_edi_ubl_bis3
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 8
## purchase_mrp
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 24
## purchase_product_matrix
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 19
## purchase_repair
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 9
## purchase_requisition
- override: 5
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 205
  · [override] Company → 'Bolag'
  · [override] Company Currency → 'Bolagets valuta'
  · [override] Company Subtotal → 'Bolag Subtotal'
  · [override] Company Total → 'Bolag Totalt'
  · [override] You cannot change the Agreement Type or Company of a not dra → 'Du kan inte ändra avtalstyp eller bolag '
## purchase_requisition_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## purchase_requisition_stock
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 14
## purchase_stock
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 220
  · [override] Companies → 'Bolag'
  · [override] Put an address if you want to deliver directly from the vend → 'Ange en adress om du vill leverera direk'
## rating
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 99
  · [override] " or someone from the same company can give it a rating. → '" eller så kan någon från samma bolag ge'
## repair
- override: 1
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 309
  · [override] Company → 'Bolag'
## resource
- override: 5
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 169
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] If empty, this is a generic time off for the company. If a r → 'Om det är tomt är detta en generisk ledi'
  · [override] Number of hours to work on the company schedule to be consid → 'Antal timmar att arbeta enligt bolagets '
  · [override] When enabled, it will allow employees to work flexibly, with → 'När det är aktiverat kommer det att till'
## resource_mail
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 18
## sale
- override: 11
- converged 🎉: 2
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 992
  · [override] Changing the company of an existing quotation might need som → 'Att ändra bolag för en befintlig offert '
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Document Layout → 'Layout för bolagsdokument'
  · [override] Default on whether the sales price used on the product and i → 'Standard om huruvida det försäljningspri'
  · [override] The company is required, please select one before making any → 'Bolaget är obligatoriskt, vänligen välj '
  · [override] The following products cannot be restricted to the company % → 'Följande produkter kan inte begränsas ti'
  · [override] There does not seem to be any discount product configured fo → 'Det verkar inte finnas någon rabattprodu'
  · [override] Warning for the change of your quotation's company → 'Varning för byte av bolag för din offert'
  · [override] Your quotation contains products from company %(product_comp → 'Ditt erbjudande innehåller produkter frå'
  · [override] <span class="fa fa-lg fa-building-o p-2" title="Values set h → '<span class="fa fa-lg fa-building-o p-2"'
## sale_crm
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 41
## sale_edi_ubl
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 7
## sale_expense
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 23
## sale_expense_margin
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## sale_gelato
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 55
  · [override] Companies → 'Bolag'
## sale_gelato_stock
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## sale_loyalty
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 99
## sale_loyalty_delivery
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 12
## sale_management
- override: 4
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 103
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Your template cannot contain products from multiple companie → 'Din mall kan inte innehålla produkter fr'
  · [override] Your template contains products from company %(product_compa → 'Din mall innehåller produkter från bolag'
## sale_margin
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 8
## sale_mrp
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 27
## sale_pdf_quote_builder
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 118
  · [override] Company → 'Bolag'
  · [override] Provide header pages and footer pages to compose an attracti → 'Tillhandahåll sidhuvud och sidfot för at'
## sale_product_matrix
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 16
## sale_project
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 139
## sale_project_stock
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 12
## sale_project_stock_account
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## sale_purchase
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 35
## sale_purchase_project
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## sale_purchase_stock
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 7
## sale_service
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## sale_stock
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 152
  · [override] Companies → 'Bolag'
  · [override] You must have a warehouse for line using a delivery in diffe → 'Du måste ha ett lager för linje som anvä'
## sale_stock_margin
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## sale_stock_product_expiry
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 8
## sale_timesheet
- override: 5
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 263
  · [override] As an employee of our company, you will <b>collaborate with  → 'Som anställd på vårt bolag kommer du att'
  · [override] Each employee has a chance to see the impact of his work.
   → 'Varje anställd har en chans att se effek'
  · [override] Real responsibilities and challenges in a fast evolving comp → 'Verkligt ansvar och utmaningar i ett sna'
  · [override] Sales order item that will be selected by default on the tim → 'Försäljningsorderobjekt som kommer att v'
  · [override] Great quotation templates will significantly
                → 'Grymma offertmallar kommer definitivt at'
## sale_timesheet_margin
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## sales_team
- override: 6
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 127
  · [override] <i class="fa fa-fw me-1 fa-building text-primary" title="Com → '<i class="fa fa-fw me-1 fa-building text'
  · [override] Company → 'Bolag'
  · [override] Member Company → 'Medlemsbolag'
  · [override] The default company for this user. → 'Standardbolaget för den här användaren.'
  · [override] UX: Limit to team company or all if no company → 'UX: Begränsa till teambolag eller alla o'
  · [override] User Company → 'Användarbolag'
## sms
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 272
  · [override] Companies → 'Bolag'
  · [override] Your sender name must be between 3 and 11 characters long an → 'Ditt avsändarnamn måste vara mellan 3 oc'
## sms_twilio
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 63
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
## snailmail
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 100
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
## snailmail_account
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 14
  · [override] <span class="fa fa-lg fa-building-o" title="Values set here  → '<span class="fa fa-lg fa-building-o" tit'
## social_media
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 12
  · [override] Companies → 'Bolag'
## spreadsheet
- override: 2
- converged 🎉: 3
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 2176
  · [override] Currency not available for this company. → 'Valuta ej tillgänglig för detta bolag.'
  · [override] The company to take the exchange rate from. → 'Bolaget som valutakursen ska hämtas från'
## spreadsheet_account
- override: 4
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 47
  · [override] Companies → 'Bolag'
  · [override] The company fiscal year could not be found. → 'Bolagets räkenskapsår kunde inte hittas.'
  · [override] The company to target (Advanced). → 'Bolaget att rikta in sig på (avancerat).'
  · [override] The company. → 'Bolaget'
## spreadsheet_dashboard
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 63
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
## spreadsheet_dashboard_account
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 37
## spreadsheet_dashboard_event_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 23
## spreadsheet_dashboard_hr_expense
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 24
## spreadsheet_dashboard_hr_timesheet
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 26
## spreadsheet_dashboard_im_livechat
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 59
## spreadsheet_dashboard_pos_hr
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 23
## spreadsheet_dashboard_pos_restaurant
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 46
## spreadsheet_dashboard_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 66
## spreadsheet_dashboard_sale_timesheet
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 28
## spreadsheet_dashboard_stock_account
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 47
## spreadsheet_dashboard_website_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 22
## spreadsheet_dashboard_website_sale_slides
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 25
## stock
- override: 18
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 1932
  · [override] Changing the company of this record is forbidden at this poi → 'Det är inte tillåtet att ändra bolaget p'
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Leave this field empty if this route is shared between all c → 'Lämna detta fält tomt om rutten delas av'
  · [override] Let this field empty if this location is shared between comp → 'Lämna detta fält tomt om platsen delas a'
  · [override] My Company (Chicago) undertakes to do its best to supply per → 'Mitt bolag (Chicago) åtar sig att göra s'
  · [override] Please create a warehouse for company %s. → 'Vänligen skapa ett lager för bolaget %s.'
  · [override] Route Company → 'Ruttbolag'
  · [override] The barcode for a location must be unique per company! → 'Streckkoden för en plats måste vara unik'
  · [override] The combination of lot/serial number and product must be uni → 'Kombinationen av parti-/serienummer och '
  · [override] The company is automatically set from your user preferences. → 'Bolaget ställs automatiskt in från dina '
  · [override] The name of the warehouse must be unique per company! → 'Namnet på lagerplatsen måste vara unikt '
  · [override] The short name of the warehouse must be unique per company! → 'Kortnamnet på lagret måste vara unikt pe'
  · [override] This product's company cannot be changed as long as there ar → 'Denna produkts bolag kan inte ändras så '
  · [override] This product's company cannot be changed as long as there ar → 'Denna produkts bolag kan inte ändras så '
  · [override] You can only move positive quantities stored in locations us → 'Du kan endast flytta positiva kvantitete'
  · [override] You can't deactivate the multi-location if you have more tha → 'Du kan inte avaktivera multi-location om'
  · [override] You cannot change the company of a lot/serial number current → 'Du kan inte byta bolag för ett parti-/se'
## stock_account
- override: 6
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 202
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Currency → 'Bolagets valuta'
  · [override] Standard Price: The products are valued at their standard co → 'Standardpris: Produkterna värderas till '
  · [override] Technical field to correctly show the currently selected com → 'Tekniskt fält för att korrekt visa den v'
  · [override] Manual: The accounting entries to value the inventory are no → 'Manuell: Verifikaten för att värdera lag'
## stock_delivery
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 110
  · [override] Company → 'Bolag'
## stock_dropshipping
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 20
  · [override] Companies → 'Bolag'
## stock_fleet
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 58
## stock_landed_costs
- override: 3
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 126
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company related to this journal → 'Bolag relaterat till denna journal'
## stock_maintenance
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 10
## stock_picking_batch
- override: 4
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 221
  · [override] Company → 'Bolag'
  · [override] The selected operations should belong to a unique company. → 'De utvalda verksamheterna ska tillhöra e'
  · [override] The selected pickings should belong to an unique company. → 'Den utvalda plockningen ska tillhöra ett'
  · [override] The selected transfers should belong to a unique company. → 'De utvalda överföringarna ska tillhöra e'
## stock_sms
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 22
  · [override] Companies → 'Bolag'
## survey
- override: 3
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 999
  · [override] Company Certifications Count → 'Bolagets antal certifieringar'
  · [override] Our Company in a few questions ... → 'VårtBolag i några frågor ...'
  · [override] Quiz about our Company → 'Frågesport om vårt bolag'
## survey_crm
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 71
## transifex
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 21
## uom
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 85
## utm
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 110
## web
- override: 16
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 4612
  · [override] <strong>Company address block</strong> → '<strong>Adressblock för bolaget</strong>'
  · [override] <strong>Company details block</strong> → '<strong>Block för bolagsuppgifter</stron'
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Details → 'Bolagsuppgifter'
  · [override] Company Document Layout → 'Bolagets dokumentlayout'
  · [override] Company Logo → 'Bolagslogo'
  · [override] Company Name → 'Bolagsnamn'
  · [override] Company Tagline → 'Bolagets slogan'
  · [override] Company name → 'Bolagsnamn'
  · [override] Company tagline → 'Bolagets slogan'
  · [override] Company tagline, which is included in a printed document's h → 'Bolagets slogan, som inkluderas i ett tr'
  · [override] Contains the company address. → 'Innehåller bolagets adress.'
  · [override] Contains the company details. → 'Innehåller uppgifter om bolaget.'
  · [override] Is Company Details Empty → 'Is Bolagsuppgifter Tom'
  · [override] Values set here are company-specific. → 'Värden som anges här är bolagsspecifika.'
## web_hierarchy
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 17
## web_tour
- override: 0
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 69
## web_unsplash
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 28
## website
- override: 52
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3277
  · [override] " This company transformed our business. <br/>Their solution → '"Det här bolaget förändrade vår verksamh'
  · [override] 50,000+ companies run Odoo <br class="d-none d-lg-inline"/>t → '50 000+ bolag använder Odoo <br class="d'
  · [override] 50,000+ companies run Odoo to grow their businesses. → '50 000+ bolag använder Odoo för att utve'
  · [override] 50,000+ companies run Odoo<br/>to grow their businesses. → '50 000+ bolag använder Odoo<br/>för att '
  · [override] 50,000+ companies trust Odoo. → '50 000+ bolag litar på Odoo.'
  · [override] <b>Designed</b> for companies → '<b>Designad</b> för bolag'
  · [override] <br/>From revenue growth to customer retention and market ex → '<br/>Från intäktstillväxt till kundlojal'
  · [override] <span class="flex-grow-1">What services does your company of → '<span class="flex-grow-1">Vilka tjänster'
  · [override] <span class="s_website_form_label_content">Company</span> → '<span class="s_website_form_label_conten'
  · [override] <span class="s_website_form_label_content">Your Company</spa → '<span class="s_website_form_label_conten'
  · [override] Analyzing the numbers behind our success: <br class="d-none  → 'Objekta siffrorna bakom vår framgång: <b'
  · [override] Analyzing the numbers behind our success: an in-depth look a → 'Objekta siffrorna bakom vår framgång: en'
  · [override] Check out what's new in our company ! → 'Kolla in vad som är nytt i vårt bolag!'
  · [override] Companies → 'Bolag'
  · [override] Company → 'Bolag'
  · [override] Company Logo → 'Bolagslogo'
  · [override] Contact us about anything related to our company or services → 'Kontakta oss om allt som rör vårt bolag '
  · [override] Created in 2021, the company is young and dynamic. Discover  → 'Bolaget bildades 2021 och är ungt och dy'
  · [override] Dive deeper into our company’s abilities. → 'Dyk djupare in i vårt bolags förmågor.'
  · [override] Founder and chief visionary, Tony is the driving force behin → 'Tony är bolagets grundare och främsta vi'
  · [override] Founder and chief visionary, Tony is the driving force behin → 'Tony är bolagets grundare och visionär o'
  · [override] Founder, Tony is the driving force behind the company. He lo → 'Grundaren Tony är den drivande kraften b'
  · [override] Founder, Tony is the driving force behind the company. He lo → 'Grundaren Tony är den drivande kraften b'
  · [override] Here are some common questions about our company. → 'Här är några vanliga frågor om vårt bola'
  · [override] If set, add the menu as a second level menu, as a child of " → 'Om inställd, lägg till menyn som en andr'
  · [override] Info and stats about your company → 'Information och statistik om ditt bolag'
  · [override] Iris, with her international experience, helps us easily und → 'Iris, med sin internationella erfarenhet'
  · [override] James leads the tech strategy and innovation efforts at the  → 'James leder arbetet med teknisk strategi'
  · [override] Join us and make your company a better place. → 'Gå med oss och gör ditt bolag till en bä'
  · [override] Join us and make your company a better place.<br/><br/> → 'Bli en del av oss och gör ditt bolag til'
  · [override] Menu Company → 'Menybolag'
  · [override] Mich loves taking on challenges. With his multi-year experie → 'Mich älskar att ta sig an utmaningar. Me'
  · [override] Mich loves taking on challenges. With his multi-year experie → 'Mich älskar att ta sig an utmaningar. Me'
  · [override] My Company → 'Mitt Bolag'
  · [override] Our Company → 'Vårt Bolag'
  · [override] Our company → 'Vårt bolag'
  · [override] Our company specializes in consulting, product development,  → 'Vårt bolag är specialiserat på konsultve'
  · [override] The company “%(company_name)s” cannot be archived because it → 'Bolaget "%(company_name)s" kan inte arki'
  · [override] The evolution of our company → 'Utvecklingen av vårt bolag'
  · [override] Use this section to boost your company's credibility. → 'Använd detta avsnitt för att öka ditt bo'
  · [override] We are a team of passionate people whose goal is to improve  → 'Vi är ett team av passionerade människor'
  · [override] We are a team of passionate people whose goal is to improve  → 'Vi är ett team av passionerade människor'
  · [override] Website Company → 'Webbplatsbolag'
  · [override] Your Company → 'Ditt bolag'
  · [override] organization, company, people, column, members, staffs, prof → 'organisation, bolag, människor, kolumn, '
  · [override] organization, company, people, members, staffs, profiles, bi → 'organisation, bolag, människor, medlemma'
  · [override] organization, company, people, members, staffs, profiles, bi → 'organisation, bolag, människor, medlemma'
  · [override] <b>Designed</b> <br/>for Companies → '<b>Designad</b> <br/>för bolag'
  · [override] <b>My Company</b><br/>250 Executive Park Blvd, Suite 3400 <b → '<b>MittBolag</b><br/>Storgatan 1 <br/> 1'
  · [override] <span class="o_footer_copyright_name me-2">Copyright &amp;co → '<span class="o_footer_copyright_name me-'
  · [override] The company this website belongs to → 'Bolaget den här webbsidan tillhör'
  · [override] We are a team of passionate people whose goal is to improve  → 'Vi är ett team av passionerade människor'
## website_blog
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 391
  · [override] A traveler may choose to explore the area by hiking around t → 'En resenär kan välja att utforska område'
  · [override] Write a small text here to describe your blog or company. → 'Skriv en liten text här för att beskriva'
## website_cf_turnstile
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 18
## website_crm
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 29
  · [override] Your Company → 'Ditt bolag'
## website_crm_iap_reveal
- override: 8
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 82
  · [override] 1 credit is consumed per visitor matching the website traffi → '1 kredit förbrukas per besökare som uppf'
  · [override] Choose whether to track companies only or companies and thei → 'Välj om du vill spåra endast bolag eller'
  · [override] Companies → 'Bolag'
  · [override] Companies and their Contacts → 'Bolag och deras kontaktpersoner'
  · [override] Company Size → 'Bolagets storlek'
  · [override] Company Size Max → 'Bolags Maxstorlek'
  · [override] Filter companies based on their size. → 'Filtrera bolag baserat på deras storlek.'
  · [override] additional credit(s) are consumed if the company matches thi → 'ytterligare kredit(er) förbrukas om bola'
## website_crm_livechat
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 9
## website_crm_partner_assign
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 277
## website_crm_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## website_customer
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 74
## website_event
- override: 3
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 367
  · [override] Company → 'Bolag'
  · [override] The website must be from the same company as the event. → 'Webbplatsen måste vara från samma bolag '
  · [override] Use this paragraph to write a short text about your events o → 'Använd detta stycke för att skriva en ko'
## website_event_booth
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 45
## website_event_booth_exhibitor
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 26
  · [override] <span>Company Name</span>
                        <span clas → '<span>Bolagets namn</span>\n             '
  · [override] How can we showcase your company ? → 'Hur kan vi visa upp ditt bolag?'
## website_event_booth_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 11
## website_event_booth_sale_exhibitor
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 9
## website_event_crm
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 5
## website_event_exhibitor
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 160
## website_event_sale
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 33
## website_event_track
- override: 4
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 475
  · [override] Company → 'Bolag'
  · [override] Company Name → 'Bolagsnamn'
  · [override] Description of the partner (name, function and company name) → 'Beskrivning av partnern (namn, function '
  · [override] My Company global presentation → 'Mitt bolags globala presentation'
## website_event_track_live
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 16
## website_event_track_live_quiz
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 2
## website_event_track_quiz
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 103
## website_forum
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 738
## website_hr_recruitment
- override: 4
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 184
  · [override] As an employee of our company, you will <b>collaborate with  → 'Som anställd på vårt bolag kommer du att'
  · [override] Each employee has a chance to see the impact of his work.
   → 'Varje anställd har en chans att se effek'
  · [override] Real responsibilities and challenges in a fast evolving comp → 'Verkligt ansvar och utmaningar i ett sna'
  · [override] Set if the application is published on the website of the co → 'Ställ in om ansökan ska publiceras på bo'
## website_hr_recruitment_livechat
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 20
## website_links
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 66
## website_livechat
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 75
## website_mail
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 9
## website_mail_group
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 14
## website_mass_mailing
- override: 2
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 65
  · [override] Companies → 'Bolag'
  · [override] Get all the latest news, blog posts and product updates from → 'Få alla de senaste nyheterna, blogginläg'
## website_mass_mailing_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 9
## website_partner
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 19
## website_payment
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 114
## website_profile
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 107
## website_project
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 21
  · [override] Company Name → 'Bolagsnamn'
## website_sale
- override: 7
- converged 🎉: 1
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 1215
  · [override] Companies → 'Bolag'
  · [override] Company Name → 'Bolagsnamn'
  · [override] Created in 2021, the company is young and dynamic. Discover  → 'Bolaget bildades 2021 och är ungt och dy'
  · [override] Only the company's websites are allowed.
Leave the Company f → 'Endast bolagets hemsidor är tillåtna.\nLä'
  · [override] The company of the website you are trying to sell from (%(we → 'Bolaget på webbplatsen som du försöker s'
  · [override] <small class="form-text text-muted">Changing company name or → '<small class="form-text text-muted">Det '
  · [override] Changing company name is not allowed once document(s) have b → 'Det är inte tillåtet att ändra bolagsnam'
## website_sale_autocomplete
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 10
## website_sale_collect
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 50
## website_sale_collect_wishlist
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## website_sale_comparison
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 67
## website_sale_comparison_wishlist
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 1
## website_sale_gelato
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 8
## website_sale_loyalty
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 59
## website_sale_mass_mailing
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 9
## website_sale_mondialrelay
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 16
## website_sale_mrp
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
## website_sale_slides
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 55
## website_sale_stock
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 69
## website_sale_stock_wishlist
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 9
## website_sale_wishlist
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 72
## website_slides
- override: 1
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 1179
  · [override] Company Course Count → 'Bolagets kursräknare'
## website_slides_forum
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 69
## website_slides_survey
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 111
## website_sms
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 6
## website_timesheet
- override: 0
- converged 🎉: 0
- conflict ⚠️: 0
- ny (redan rätt): 0
- ny (korrigerad): 0
- ny (vår): 0
- ny (manuell): 0
- flagged 🚫: 0
- borttagen: 0
- oförändrad: 3
